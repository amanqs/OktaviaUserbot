...
from Oktavia.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if (await Oktavia.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await Oktavia.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
    ...
    try:
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await Oktavia.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await Oktavia.gban_info(user_id)):
        try:
