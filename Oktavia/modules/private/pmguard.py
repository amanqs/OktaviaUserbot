...
from Oktavia.database.pmpermitdb import get_approved_users, pm_guard
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await Oktavia.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Oktavia.set_block_message(Oktavia.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Oktavia.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Oktavia.get_pm_settings()
    await Oktavia.allow_user(chat_id)
...
@Client.on_message(filters.command(["deny", "dap", "disapprove", "d"], ["."]) & filters.me & filters.private)
